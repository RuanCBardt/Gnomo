    ...
    if (pathname.startsWith("/api/")) {
      ...
      try {
        ...
        if (request.method === "POST" && pathname === "/api/accounts") {
          ...
          return new Response(JSON.stringify(newAccount[0]), {
            status: 201,
            headers: { "Content-Type": "application/json" }
          });
        }
        if (request.method === "GET" && pathname === "/api/transactions") {
          const allTransactions = await db.select().from(transactions).orderBy(desc(transactions.date)).limit(50);
...
// .wrangler/tmp/bundle-JTN3pD/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-JTN3pD/middleware-loader.entry.ts
